...
p_relative_cases <- ggplot(df_all[df_all$type_no == 7, ],
                            aes(x=city, y=beta)) +
  scale_color_discrete(NULL) +
  geom_hline(yintercept=0, linetype="dashed") + 
  geom_point() +
  ...
  theme(axis.text.x = element_blank()) + 
  ylim(ylim1, ylim2) + 
  ggtitle(NULL,subtitle = "Relative cases") + 
  NULL



p_age <- ggplot(df_all[df_all$type_no %in% 8:10, ],
                 aes(x=city, y=beta, col=type)) +
  scale_color_discrete(NULL) +
  geom_hline(yintercept=0, linetype="dashed") + 
  geom_point() +
...
p_arrow <- ggplot() + 
  ...
  geom_text(aes(x=-0.75, y=4, label="faster decrease", angle=90), size=3) + 
  xlim(-1.4, 0.05) + 
  ylim(1.5, 6) + 
...
p_all <- wrap_plots(wrap_plots(p_arrow, p_empty, ncol=1,
                                heights=c(0.2, 1)), 
                     wrap_plots(p_dist, p_income, p_relative_cases, p_age,
                                ncol=1),
                     widths=c(0.05, 1))
ggsave(p_all, filename =
         "decreasing_model.png",
